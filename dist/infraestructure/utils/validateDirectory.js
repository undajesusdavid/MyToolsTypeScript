...
export async function obtenerPermisos(dir) {
...
export async function comprobarDirectorio(dir) {
    ...
    if (!permisos.lectura)
        throw new Error(`${prefijoDeError} El directorio ${dir} no tiene permisos de lectura`);
    if (!permisos.escritura)
        throw new Error(`${prefijoDeError} El directorio ${dir} no tiene permisos de escritura`);
    return true;
}
